...
# 避开被占用的卡0，用卡1（MoA 用 forward hook，DataParallel 不兼容）
...
class MoA(nn.Module):
    def __init__(self, dim=1024, expert_dims=EXPERT_DIMS, n_experts=NUM_EXPERTS):
        ...
        self.last_gates = None   # 存储最近一次 forward 的 gates（供负载均衡）
    def forward(self, x):
        gates = self.gate(x[:, 0, :])                              # (B, E)
        self.last_gates = gates                                    # 保持梯度，供负载均衡
        outs = torch.stack([e(x) for e in self.experts], dim=-2)   # (B, N, E, D)
        return (gates[:, None, :, None] * outs).sum(dim=-2)        # (B, N, D)
...
def encode(frames):
    ...
    合并 B*T 一次性前向提速，MoA 的 hook 对合并 batch 依然生效。"""
    B, T, C, H, W = frames.shape
    frames_flat = frames.view(B * T, C, H, W)
    out = vision(pixel_values=frames_flat)
...
def main():
    ...
    for epoch in range(1, EPOCHS+1):
        ...
        print(f"Epoch {epoch:2d}  Val F1={val_f1:.4f}  Best={best_val_f1:.4f}")

    os.makedirs(cfg.MODEL_DIR, exist_ok=True)
    torch.save(best_state, os.path.join(cfg.MODEL_DIR, "best_model.pt"))
